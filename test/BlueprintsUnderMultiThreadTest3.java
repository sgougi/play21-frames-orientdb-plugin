...
public class BlueprintsUnderMultiThreadTest3 {
	...
	public void test() throws Exception {
		...
		/* create key index */ {
			...
			graph.createIndex("c", Vertex.class);
			assertNotNull(graph.getIndex("c", Vertex.class));
		}
		
		/* create the root vertex */ {
			ODatabaseRecordThreadLocal.INSTANCE.set(graph.getRawGraph());			
			Vertex v = graph.addVertex(null);
			v.setProperty("name", "rootNode");
			graph.commit();
			rootVertexId = v.getId();
			assertNotNull(rootVertexId);
			
			List<Vertex> rootNodes = toArrayList(graph.getVertices("name", "rootNode"));
